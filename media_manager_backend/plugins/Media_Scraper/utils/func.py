...
def get_scan_dir(cfg_scan_dir: Path | None, cli_dir: str | None = None) -> str | None:
    ...
    if cli_dir:
        ...
        if cli_path.exists():
            ...
            logger.error(f"命令行指定的目录无效：'{cli_dir}'")
            return None
    ...
    if cfg_scan_dir:
        if cfg_scan_dir.exists():
            ...
            return None  # 添加 return，避免继续执行到弹窗部分
    
    # 只有在没有命令行参数且没有配置目录时才弹窗
